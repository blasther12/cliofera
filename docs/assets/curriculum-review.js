    ...
    const apply = () => {
      ...
      function timelineHtml(filter = '') {
        ...
        const eventHtml = events.map(event => {
          const related = (event.courses || []).map(id => state.data.courses.find(course => course.id === id)).filter(Boolean).map(course => `<a href="#/course/${safe(course.id)}">${safe(course.title)}</a>`).join('');
      ...
      function renderTimeline() {
        ...
        if (input) input.addEventListener('input', () => {
          ...
          app.innerHTML = timelineHtml(value);
          const next = document.getElementById('timelineSearch');
          if (next) {
            next.focus();
            next.setSelectionRange(value.length, value.length);
            next.addEventListener('input', () => renderTimelineFiltered(next.value));
          }
        }, { once: true });
        if (typeof installUI === 'function') installUI();
      }

      function renderTimelineFiltered(value) {
        const app = document.getElementById('app');
        if (!app) return;
        app.innerHTML = timelineHtml(value);
        const input = document.getElementById('timelineSearch');
        if (input) {
          ...
          input.addEventListener('input', event => renderTimelineFiltered(event.target.value));
        }
      }
